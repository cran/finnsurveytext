...
Additionally, the `fst_find_stopwords()` function can be used to find currently available lists of  stopwords for exclusion from the data. (The default "language" is "fi", but ) The "name" column can be used to choose a list for the `stopword_list` variable above. 
