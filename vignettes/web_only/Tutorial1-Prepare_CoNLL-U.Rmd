...
The `finnsurveytext` package uses the CoNLL-U format. This tutorial demonstrates the process of preparing Finnish survey text data into this format using functions in r/01_prepare_conll-u.r.
...
```{r eval = FALSE}
prepd_bullying <- fst_prepare_conllu(
  data = child_barometer_data,
  field = "q7",
  stopword_list = "nltk",
  model = "ftb"
)
...
As an example, the development cooperation q11_2 data could be prepared using this function call: 

```{r eval = FALSE}
prepd_lonely <- fst_prepare_conllu(
  dev_data,
  field = "q11_2",
  stopword_list = "none",
  model = "tdt"
...
* The `field` is the name of the column in your data which contains the open-ended survey question. In this example, the responses are in question 11_2. 
* We have chosen *not* to remove stopwords and punctuation in this example. (`stopword_list = NULL`)
...
* Tutorial2-Data exploration
* Tutorial3-Overview of Concept Network functions
* Tutorial4-Further analysis and configuration of Concept Network functions
* Tutorial5-Demonstration of comparison functions
* Tutorial6-Example of analysis of an open-ended survey question
